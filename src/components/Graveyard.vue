<template>
    <div class="flex">
        <div class="relative flex" @mouseenter="hovering = true" @mouseleave="hovering = false">
            ...
            <img v-if="graveyard.length > 0" :src="topCard.image" alt="Graveyard Card"
                class="absolute z-10 w-48 rounded-xl">
        </div>
    </div>
</template>
...
const topCard = computed(() => graveyard.value[graveyard.value.length - 1]);

// Data
const hovering = ref(false);

// Methods
const showGraveyardModal = () => $evt.emit('show-graveyard-modal');
</script>
